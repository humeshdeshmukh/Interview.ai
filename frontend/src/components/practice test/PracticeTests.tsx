...
import './PracticeTests.css'; // Import the CSS file
import questionsData from './questions.json'; // Import the questions JSON file
...
const PracticeTests: React.FC = () => {
  ...
  const calculateScore = () => {
    return selectedAnswers.reduce((score, answer, index) => {
      return score + (answer === questionsData[index].correctAnswer ? 1 : 0);
    }, 0);
  };
      ...
      {isSubmitted ? (
        ...
        <>
            ...
            {questionsData[currentQuestionIndex].options.map((option, index) => (
              <li key={index}>
                <button
                  ...
                  style={{ background: selectedAnswers[currentQuestionIndex] === option ? 'lightblue' : '' }}
                >
                  {option}
                </button>
              </li>
          ...
          <div>
            ...
            <button onClick={handleNextQuestion} disabled={currentQuestionIndex === questionsData.length - 1}>
              Next Question
            </button>
